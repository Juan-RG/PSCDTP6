...
void LindaDriver::RN_2(const Tupla p1, const Tupla p2, Tupla &t1, Tupla &t2) {
    ...
    if ((p1.size() == p2.size()) && (t1.size() == t2.size()) &&
        (p1.size() == t2.size())) { // Si las tuplas son de diferente tamaño,
...
void LindaDriver::RDN_2(const Tupla p1, const Tupla p2, Tupla &t1,
                        Tupla &t2) { // Exactamente igual que RN_2 pero sin borrarlas al final
    string mensaje;
    int read_bytes;   // num de bytes recibidos en un mensaje
    int send_bytes;   // num de bytes enviados en un mensaje
    string buffer;
